...
import { ButtonComp, CheckboxComp, InputField, TextComp } from "../../basic-components";
import { dimensions, fontSizes, color, fontWeights } from "../../styles";
import { useTranslation } from "react-i18next";
import { Modal } from "react-native-paper";
import { View, StyleSheet, ScrollView, Image, Platform, TextInput } from "react-native";
import { AgencyKeyGetList, AttachmentType, KeysProcessStep, ProductionExperienceTypeValueStatic, TypeLevelUser } from "../../utils/constant/key";
...
import { ModalSelectAgencySend } from "../modal-select-agency-send";
...
export function ModalRequest({
    ...
    const [showSelectAgency, setShowSelectAgency] = useState(false);
    ...
    useEffect(() => {
        ...
        setName('');
        setIndexView(0);
        if (userInfo.user_level == TypeLevelUser.agency) setAgency({ id: userInfo.agency_id || 0, name: userInfo.agency_nm });

    }, []);

    const addRequest = (title, content, all, agency_id) => {
        setLoading(true);
        IO.sendRequest(
            serviceList.insert_fm_request,
            [
                title,
                content,
                all,
            ...
            (requestInfo, result) => callBackAddRequest(requestInfo, result),
            true,
            timeOutFunct,
            15000,
            "insert_fm_request"
    ...
    const callBackAddRequest = (requestInfo, result) => {
    ...
    const openImagePicker = () => {
        ...
        launchImageLibrary(options, async (response) => {
            if (response.didCancel || response.error) {
                ...
                (response.assets || []).forEach(async (image, index)=>{
        ...
        >
            <View style={styles.cardModal}>
                ...
                <ScrollView style={{ margin: dimensions.moderate(10), zIndex: 999999999 }}>
                    ...
                    {
                        userInfo.user_level == TypeLevelUser.farmer && <TouchableWithoutFeedback onPress={() => { Keyboard.dismiss; }}>


                            <InputField
                                onPressIn={() => {
                                    setShowSelectAgency(true);
                                }}
                    ...
                    {userInfo.user_level == TypeLevelUser.agency && <View >
                        <CheckboxComp
                            type="checkbox"
                            checked={sendAll}
                ...
                >
                    ...
                    <ButtonComp
                        ...
                        mode="add"
                        loading={loading}
                        disabled={!title.length || !content.length || !agency || !agency.id}
                        onPress={() => {
                            addRequest(title, content, sendAll ? '1' : '0', agency.id);
            ...
            <ModalSelectAgencySend
                btnText={"reject"}
                closeModal={(status, item) => {
                    setShowSelectAgency(false);
                    if (status) setAgency({
                        id: item[AgencyKeyGetList.id],
                        name: item[AgencyKeyGetList.name]
                    });
                }}
                farmer_id={userInfo.farmer_id}
                focus={true}
                visibleModal={showSelectAgency}
                notify={(data) => {
                    notify(data)
                }}
            ></ModalSelectAgencySend>
