...
import { KeyGetListRealisticCrops, TypeStaticContracting } from "../../../../../utils/constant/key";
import RowTree from "./row-tree/row-tree";

export default function TreeScreen({ navigation, route, category, notify, id }) {
    ...
    const [list_tree, setList] = useState([]);
    ...
    const handleScroll = ({ nativeEvent }) => {
        ...
        if (isScrollEnd && !loading) {
            if (!list_tree.length || list_tree.length >= totalRow) return;
            getList(list_tree[list_tree.length - 1][KeyGetListRealisticCrops.id], list_tree); // last_id
        }
    }
    ...
    const getList = (lastId, list) => {
        ...
        setLoading(true);
        IO.sendRequest(
            serviceList.get_list_realistic_crops,
            [
                lastId, //last_id
                id,//contract_id
                0,//land_plot_id:
            ...
            >
                ...
                <View style={{ flex: 1, marginBottom: dimensions.vertical(10) }}>
                    <TextComp fontWeights={fontWeights.light}
                        ...
                        value={`~ ${list_tree.length}/${totalRow} ~`}
                    ></TextComp>
                </View>
