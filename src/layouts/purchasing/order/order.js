...
import RowOrder, { KeyGetListPurchasing } from "./row-order/row-order";


export default function OrderScreen({ navigation, route, category, from_dt, to_dt, notify , onModalDetail, search}) {
    ...
    useEffect(() => {
        if (debouncedSearchParams) {
            getListOrderPurchase(1000000000000000000, []);
        } 
    ...
    const handleScroll = ({ nativeEvent }) => {
        ...
        if (isScrollEnd && !loading) {
            ...
            getListOrderPurchase(list_item[list_item.length - 1][KeyGetListPurchasing.id], list_item); // last_id
        }
    }

    const getListOrderPurchase = (lastId, list) => {
        if (loading) return;
        setLoading(true);
        IO.sendRequest(
            serviceList.get_list_purchasing,
            [
                lastId, //last_id
                `%${(search || '').trim()}%`, //search
            ...
            (requestInfo, result) => callBackGetListOrderPurchase(requestInfo, result, list),
            true,
            timeOutFunct,
            15000,
        );
    ...
    const callBackGetListOrderPurchase = (requestInfo, result, list) => {
    ...
    const fetchData = () => {
        setTimeout(() => {
            getListOrderPurchase(1000000000000000, []);
