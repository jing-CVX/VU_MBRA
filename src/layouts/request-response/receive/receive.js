...
import RowReceive from "./row-receive/row-receive";


export default function ReceiveScreen({ navigation, route, category, from_dt, to_dt, notify, search}) {
   ...
   const [debouncedSearchParams, setDebouncedSearchParams] = useState({ search: '', from_dt: '', to_dt: '' });
    ...
    const getList = (lastId, list) => {
        ...
        setLoading(true);
        IO.sendRequest(
            userInfo.user_level == TypeLevelUser.farmer ? serviceList.get_list_agency_send_fm_request : serviceList.get_list_farmer_send_fm_request,
            [
                lastId, //last_id
                `${(search || '').trim()}`, //search
                        ...
                        <View style={styles.list_item_box}>
                            {(list_item || []).map((item, index) => {
                                return <View key={index}>
                                    <TouchableOpacity 
                                        onPress={()=>{
                                            navigation.navigate(screens.DetailReceiveScreen,
                                                {
                                    ...
                                    >
                                        <RowReceive item={item}></RowReceive>
                                    </TouchableOpacity>
                                </View>;
                            })}
