...
import { KeyGetListPurchaseAnnualQuota, TypeStaticContracting } from "../../../utils/constant/key";
import RowRemain from "./row-remain/row-remain";


export default function ContractingRemainScreen({ navigation, route, category, from_dt, to_dt, notify, search }) {
    ...
    const [_list_remain, setListRemain] = useState([]);
    ...
    const handleScroll = ({ nativeEvent }) => {
        ...
        if (isScrollEnd && !loading) {
            if (!_list_remain.length || _list_remain.length >= totalRemain) return;
            getListLarkContracting(_list_remain[_list_remain.length - 1][KeyGetListPurchaseAnnualQuota.id], _list_remain); // last_id
        }
    }
    ...
    const getListLarkContracting = (lastId, list) => {
        ...
        IO.sendRequest(
            ...
            [
                ...
                0,//land_plot_id:
                0,//tree_id,
                moment(from_dt, 'DD/MM/YYYY').format('YYYY'), //  date_start
                moment(to_dt, 'DD/MM/YYYY').format('YYYY'),  //date_end
                userInfo.agency_id || 0,//agency_id
                TypeStaticContracting.plus,//type:
                0,//company id
                0,//branch_id
                `%${(search || '').trim()}%`
            ],
    ...
    const callBackGetListLarkContracting = (requestInfo, result, list) => {
        ...
        if (+result["proc_status"] == 1) {
            ...
            setTotalRemain(result["proc_data"]["rowTotal"] || 0);
            setListRemain(list.concat(data || []));
        } else {
            notify({
                show: true,
                titleModal: message,
        ...
        <>
          <View style={[{display:'flex' ,flexDirection:'column', paddingRight:dimensions.moderate(10)}]}>
                    <TextComp fontWeights={fontWeights.light}
                                ...
                                value={`~ ${_list_remain.length}/${totalRemain} ~`}
                            ></TextComp>
                    </View>
                ...
                <View style={{ display: "flex", flexDirection: "column", }}>
                    <View>
                        <View style={styles.list_item_box}>
                            {(_list_remain || []).map((item, index) => {
                                return <View key={index}>
                                    <TouchableOpacity>
                                        <RowRemain item={item}></RowRemain>
                                    </TouchableOpacity>
                                </View>;
                            })}
