...
import RowLark from "./row-lark/row-lark";
import { KeyGetListPurchaseAnnualQuota, TypeStaticContracting } from "../../../utils/constant/key";


export default function ContractingLarkScreen({ navigation, route, category, from_dt, to_dt, notify , search}) {
    ...
    const handleScroll = ({ nativeEvent }) => {
        ...
        if (isScrollEnd && !loading) {
            ...
            getListLarkContracting(_list_lark[_list_lark.length - 1][KeyGetListPurchaseAnnualQuota.id], _list_lark); // last_id
        }
    }
    ...
    const getListLarkContracting = (lastId, list) => {
        ...
        IO.sendRequest(
            ...
            [
                ...
                TypeStaticContracting.minus,//type:
                0,//company id
                0,//branch_id
                `%${(search || '').trim()}%`
            ],
